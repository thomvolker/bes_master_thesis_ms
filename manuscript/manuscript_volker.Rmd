...
footnote:
  ...
  - code: 2
    text: "FETC of Utrecht University granted ethical approval for this work."
keywords:
  - Bayes factors
  - replication
...
This approach entails that a non-informative improper prior is updated with a small fraction $b$ of the information in the data (i.e., the likelihood), resulting in a proper default prior distribution [e.g., @mulder_equality_2010; @gu_approximated_2018], such that
$$
p_u(\beta, \phi|Y, X, b) = 
\frac{
...
That is, $p_i(\beta | Y, X, b) \propto p_u(\beta | Y, X, b)\boldsymbol{1}_{\boldsymbol{B_i}}$, where $\boldsymbol{1}_{\boldsymbol{B_i}}$ is an indicator for the admissible parameter space under hypothesis $H_i$ [@gu_approximated_2018].
...
```{r BF34, echo=F, message=F, warning=F, fig.height=3.8, out.width='100%', dpi=800, fig.cap = "Aggregated Bayes factors for hypothesis $H_3: \\beta_6 > 0$ versus $H_u$ in simulation 3 and $H_4: \\beta_{\\text{low}} < \\beta_{\\text{medium}} < \\beta_{\\text{high}}$ versus $H_u$ in simulation 4 over three studies (with linear, logistic and probit models)."}
...
```{r PMP34, echo=F, message=F, warning=F, fig.height=3.8, out.width='100%', dpi=800, fig.cap = "Aggregated $PMP$s for hypothesis $H_3: \\beta_6 > 0$ versus $H_u$ and $H_c$ in simulation 3, and for $H_4: \\beta_{\\text{low}} < \\beta_{\\text{medium}} < \\beta_{\\text{high}}$ versus $H_u$ or $H_c$ in simulation 4 over three studies (with linear, logistic and probit models)."}
wrap_plots(PMP3 + 
...
```{r BF56, echo=F, message=F, warning=F, fig.height=3.8, out.width='100%', dpi=800, fig.cap = "Aggregated Bayes factors for hypothesis $H_5: \\beta_{\\text{scale}} > 0$ versus $H_u$ in simulation 5 and $H_6: \\{\\beta_2, \\beta_3, \\beta_4\\} > 0$ versus $H_u$ in simulation 6 over three studies (with linear, logistic and probit models)."}
wrap_plots(BF5 + labs(title = "Simulation 5"), 
           BF6 + labs(title = "Simulation 6"), ncol = 1)
```

```{r PMP56, echo=F, message=F, warning=F, fig.height=3.8, out.width='100%', dpi=800, fig.cap = "Aggregated $PMP$s for hypothesis $H_5: \\beta_{\\text{scale}} > 0$ versus $H_u$ and $H_c$ in simulation 5 and $H_6: \\{\\beta_2, \\beta_3, \\beta_4\\} > 0$ versus $H_u$ or $H_c$ in simulation 6 over three studies (with linear, logistic and probit models)."}
wrap_plots(PMP5 + 
...
```{r BF78, echo=F, message=F, warning=F, fig.height=3.80, out.width='100%', dpi=800, fig.cap = "Aggregated Bayes factors for the \\textit{incorrect} hypothesis $H_7: \\{\\beta_2, \\beta_3, \\beta_4\\} < 0$ versus $H_u$ in simulation 7 and \\textit{partially incorrect} hypothesis $H_8: \\{\\beta_1, \\beta_2, \\beta_3\\} > 0$ versus $H_u$ in simulation 8 over three studies (with linear, logistic and probit models). Note the different scaling of the y-axis."}
wrap_plots(BF7 + labs(title = "Simulation 7"), 
           BF8 + labs(title = "Simulation 8"), ncol = 1)
```

```{r PMP78, echo=F, message=F, warning=F, fig.height=3.80, out.width='100%', dpi=800, fig.cap = "Aggregated $PMP$s for the \\textit{incorrect} hypothesis $H_7: \\{\\beta_2, \\beta_3, \\beta_4\\} < 0$ versus $H_u$ or $H_c$ in simulation 7 and $H_8: \\{\\beta_1, \\beta_2, \\beta_3\\} > 0$ versus $H_u$ or $H_c$ in simulation 8 over three studies (with linear, logistic and probit models). Note that the line of $PMP_{i,u}$ in simulation 7 disappears because it is completely overlayed by the line for $PMP_{i,c}$."}
wrap_plots(PMP7 + 
...
Simulation 11 evaluates the *correct* $H_{11}: \{\beta_2, \beta_3, \beta_4\} > 0$ ($\beta_2 = \beta_3 = \beta_4 = 0.054$ in the population; Table \@ref(tab:coefs)), which is the same hypothesis as evaluated in simulation 6.
...
```{r simBF11, eval=F, fig.show='hide', include=F, echo=F, message=F, dpi=400, dev='png', fig.height=2.1, out.width="100%", cache=T, fig.pos='!t', fig.cap = "Cumulative aggregated Bayes factors for hypothesis $H_{11}: \\{\\beta_2, \\beta_3, \\beta_4\\} > 0$ in simulation 11 versus $H_u$ and $H_c$ over 1 to 150 studies. In simulation 12, the hypothesis space is separated in three parts ($H_{12_a}: \\beta_2 > 0$, $H_{12_b}: \\beta_3 > 0$ and  $H_{12_c}: \\beta_4 > 0$), evaluated against their respective complements and an unconstrained hypothesis."}
